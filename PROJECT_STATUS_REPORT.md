...
> 2026-09-21 시가총액 갱신: 최신 평형 마스터 511개 단지·3,435개 타입과 KB 가격을 다시 반영했습니다. KB 시세만 385개 단지(약 165.09조원), 보정 추정 포함 421개 단지(약 194.49조원)입니다. 해운대두산위브더제니스는 KB 미고시를 검증된 최근 실거래 평형가격으로 구분 보정해 약 34,731.75억원, 남천자이는 KB 부분합계와 펜트하우스 면적단가 보정을 합산해 약 13,863.30억원으로 표시합니다. 보정 정책과 최종 snapshot 생성 책임을 `area_master`로 이전하고 웹서비스에는 검증 동기화 단계만 남겼습니다. 최신 run ID는 `285aaf0c2b23697c9d41f95b`이며 전체 테스트 145개를 통과했습니다.

> 2026-09-18 후속 보정: 더비치푸르지오써밋 약 17,269.42억원, 더샵남천프레스티지 약 11,776.26억원을 면적 단가 보정 추정치로 추가했습니다. KB 화면 기본 ‘보정 추정 포함’은 362개 단지, ‘KB 시세만’은 360개입니다. 보정 대상·근거와 원본 가격을 구분하며 시가총액 관련 테스트 37개를 통과했습니다.

> 작성 기준일: 2026-09-12  
> 분석 기준: 현재 저장소의 코드, 설정, Git 이력, 테스트 및 저장된 데이터 산출물  
> 대상 독자: 이 프로젝트를 처음 실행하거나 유지보수하는 사용자

> **2026-09-18 시가총액 보정:** KB 수집 배포본 497개 단지·3,308개 타입을 연결했습니다. `아파트 시가총액` 메뉴의 기본 KB 모드에서 360개 단지(합계 약 147.22조원)를 산정하고, 평형별 세대수·가격·출처·보완 사유를 조회합니다. KB 시세 기준일이 없어 수집일 기준으로 표시합니다. 실거래 모드는 2026-08을 재산정했으며 A·B 34개, C·D 포함 완전 산정 120개입니다. 과거 월 일괄 재구성은 별도 실행합니다. 전체 테스트 117개 통과. 자세한 내용은 [시가총액 구현·운영 보고서](reports/MARKET_CAP_IMPLEMENTATION.md)를 참고하세요.
