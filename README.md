...
학교 지도 레이어를 처음 준비하거나 학교 분석 결과를 갱신한 뒤에는 명시적으로 조회용 snapshot을 만듭니다.

```powershell
python main.py sync-school-data --source ../busan_school_analysis
```

동기화는 공식 학교 ID로 기본정보·점수·연도별 자료를 연결하고 검증이 끝난 파일만
`data/processed/schools/`에 저장합니다. 앱은 이 snapshot만 읽으므로 배포 환경에 형제 학교 프로젝트가
없어도 동작하며, 앱 실행이나 rerun 중에 동기화를 자동 수행하지 않습니다. 원천 경로·SHA-256·기준연도·행 수·동기화 시각은
`data/processed/schools/manifest.json`에 기록됩니다. 초등학교 수요점수와 중학교 점수는 서로 다른 지표이며 합산하지 않습니다.
...
`실거래가 단지 검색` 메뉴에서도 동일한 `지역` 필터를 사용하며, 김해·양산 선택 시
`transactions_master.parquet`의 최신 1~3개월 거래와 광역 단지정보를 검색합니다.
...
현재 배포본의 511개 단지·3,435개 타입을 반영했으며, KB 가격과 세대수가 모두 확인된 385개 단지를 산정했습니다.
