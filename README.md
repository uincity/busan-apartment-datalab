...
부산광역시 16개 구·군의 국토교통부 아파트 매매 실거래와 K-apt 단지정보를 결합해 부산 → 구·군 → 법정동 → 단지 → 면적그룹 → 거래로 탐색하는 재사용 가능한 분석 프로젝트입니다.
...
실거래 수집은 부산 16개 지역 × 월을 순회합니다. 저장된 지역/월 파일은 건너뛰며 다시 받을 때만 `--force`를 사용합니다. API 호출은 페이지네이션, timeout, retry, exponential backoff를 적용하고 한 지역의 빈 응답·실패가 전체 수집을 중단하지 않습니다.
...
메뉴는 부산 Overview, 구군 비교, 동 비교, 아파트 상세, 아파트 비교(최대 5개), 시장회복 Watch, 노후단지 Watch로 구성됩니다. 기간·구군·법정동·단지·면적그룹·연식·세대수 필터를 제공합니다. 최신 잠정 월이 포함되면 화면 하단에 표시합니다.
...
핵심 출력은 `busan_apartment_monthly.parquet`, `busan_district_summary.csv`, `busan_dong_summary.csv`, `busan_complex_summary.csv`, `representative_complexes.csv`, `recovery_watchlist.csv`, `old_apartment_watchlist.csv`, `apartment_match_log.csv`, `reports/data_quality_report.csv`입니다.
...
현재 구현은 매매 실거래/K-apt 수집, 정제, 3단계 매칭, 월 패널, 회전율·가격 추세, 3종 요약, 대표단지 점수, 두 Watch 목록, 품질검증, Plotly/Streamlit 탐색까지입니다.

아직 구현하지 않은 확장 기능은 전월세, 지하철·학교·학군·상권·해안·공원·고도, 입주물량, 인구, 한국부동산원 지수와 가격예측 모델입니다. 다음 개발 우선순위는 ① 실제 API 응답 샘플로 K-apt 필드 별칭 회귀검증, ② 수동 매칭 보정 테이블, ③ 공간정보 플러그인 계층, ④ 시계열 기반 모델 검증 순입니다.
