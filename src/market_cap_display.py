...
def render_kb_market_cap(output: Path):
    ...
    with st.container(horizontal=True):
        ...
        st.metric("산정 세대수", f"{complete.priced_households.sum():,.0f}세대", border=True)
        st.metric("보완 대상", f"{len(cohort) - len(complete):,}개", border=True)
        value = complete.market_cap_krw.sum(min_count=1)
        st.metric("산정 완료 단지 합계", f"{value / 1e8:,.0f}억원" if pd.notna(value) else "산정 불가", border=True)
    ...
    columns = ["rank", "district_rank", "complex_name", "sigungu", "dong", "households", "eligible_households", "priced_households", "valuation_scope", "cap_eok", "value_eok"]
    if shown.empty:
        st.info("현재 조건에서 전체 산정 범위의 가격을 확보한 단지가 없습니다.")
    else:
        st.dataframe(shown[columns].rename(columns=labels), hide_index=True)
    ...
    with st.expander("단지별 평형·세대수·KB 시세", expanded=True):
        ...
        if choices.empty:
            ...
            st.dataframe(pd.DataFrame([item])[["households", "eligible_households", "confirmed_households", "priced_households", "valuation_scope", "partial_cap_krw"]].rename(columns=labels), hide_index=True)
            if detail.empty:
                ...
                st.dataframe(detail[fields].rename(columns=labels), hide_index=True)
    ...
    with st.expander("보완 목록 · 산정 근거"):
        ...
        st.write("KB 단지·타입이 하나로 연결되고 전용면적·공급면적·타입명·세대수가 일치하는 자료만 사용합니다. 원본 만원 단위를 원으로 환산하며, 타입별 가격 차이를 유지합니다. 시세 누락·가격 역전·세대수 불일치는 임의 보정하지 않습니다.")
        st.json(metadata)
...
def render_market_cap():
    ...
    with st.container(horizontal=True):
        ...
        grades = st.multiselect("산정 품질", ["A", "B", "C", "D"], default=["A", "B"], key="cap_grades")
    ...
    with st.container(horizontal=True):
        ...
        st.metric("랭킹 포함 단지 합계", f"{total / 1e8:,.0f}억원" if pd.notna(total) else "산정 불가", border=True)
    ...
    with st.expander("단지 상세 · 월별 추이 · 사용 거래", expanded=False):
        ...
        st.dataframe(pd.DataFrame([c])[ ["households", "confirmed_households", "priced_households", "household_coverage", "price_coverage", "partial_cap_krw", "grade"] ].rename(columns=LABELS), hide_index=True)
        ...
        if areas.empty:
            ...
            if not areas.empty:
                areas["contribution_share"] = areas.contribution_krw / areas.contribution_krw.sum(min_count=1)
                areas["price_difference_1m_pct"] = (areas.price_1m_krw / areas.price_krw - 1) * 100
                st.dataframe(areas.drop(columns=["used_trade_ids"]).rename(columns=LABELS), hide_index=True)
                ...
                st.dataframe(used[["source_row_id", "deal_date", "area_sqm", "floor", "deal_amount_krw", "direct_trade", "unusual_price", "same_terms_multiple"]], hide_index=True)
